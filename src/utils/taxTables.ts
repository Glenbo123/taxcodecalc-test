...
import { preciseAdd, preciseSubtract, preciseMultiply, preciseDivide, preciseRound } from './precisionCalculations';
...
export function calculateIncomeTax(annualSalary: number, taxCode: string): number {
  debugTax('Calculating income tax - salary: %d, taxCode: %s', annualSalary, taxCode);
  ...
  if (annualSalary > 100000) {
    ...
    const reduction = Math.min(personalAllowance, Math.floor(preciseDivide(excessIncome, 2)));
  ...
  const taxableIncome = Math.max(0, preciseSubtract(annualSalary, personalAllowance));
...
export function calculateNI(monthlySalary: number): number {
  ...
  if (monthlySalary > monthlyUpperLimit) {
    const higherRateEarnings = preciseSubtract(monthlySalary, monthlyUpperLimit);
    niContribution = preciseAdd(
      niContribution,
 ...
 */
export function calculateTotalTax(annualSalary: number, taxCode: string): number {
  // Validate inputs
  if (!annualSalary || isNaN(annualSalary) || annualSalary <= 0) {
    return 0;
  }
 ...
 */
export function calculateTakeHomePay(annualSalary: number, taxCode: string): number {
  // Validate inputs
  if (!annualSalary || isNaN(annualSalary) || annualSalary <= 0) {
    return 0;
  }
...
export function getTaxCodeDescription(taxCode: string): string {
  ...
  if (['BR', 'D0', 'D1', 'NT', '0T'].includes(normalizedCode)) {
    ...
    if (isNegativeAllowance) {
      description += 'Reduced allowance due to benefits or previous underpaid tax (K code). ';
    } else {
