  ...
  const checkEligibility = (): BenefitEligibility[] => {
    ...
    if (isCaringForSomeone) {
      benefits.push({
        ...
        description: 'A payment for people who care for someone for at least 35 hours a week',
        amount: 76.75,
        frequency: 'weekly'
      });
    }
    ...
    if (isDisabled) {
      benefits.push({
        ...
        eligible: true,
        description: 'A payment to help with extra living costs if you have a long-term health condition or disability',
        amount: 172.75,
        frequency: 'weekly'
      });
    }
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-center gap-2 mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Benefits Calculator</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="mt-1 relative rounded-md shadow-sm">
                ...
                <input
                  type="number"
                  value={housingCosts}
                  onChange={(e) => setHousingCosts(parseFloat(e.target.value) || 0)}
                  className="focus:ring-govuk-blue focus:border-govuk-blue block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md dark:bg-gray-800 dark:border-gray-700 dark:text-white"
                />
              </div>
            </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Potential Benefits</h2>
            <div className="space-y-4">
              {benefits.map((benefit, index) => (
                ...
                >
                  <h3 className="text-sm font-medium text-gray-900 dark:text-white">{benefit.name}</h3>
                  ...
                  {benefit.eligible && benefit.amount && (
                    <p className="text-sm font-medium text-green-700 dark:text-green-400 mt-2">
                      Estimated amount: £{benefit.amount.toFixed(2)} {benefit.frequency}
                    </p>
                  )}
                  {!benefit.eligible && (
