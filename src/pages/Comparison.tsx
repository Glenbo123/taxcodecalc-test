...
import { PlusIcon, ArrowsRightLeftIcon, InformationCircleIcon, ChartBarIcon } from '@heroicons/react/24/outline';
...
export function Comparison() {
  ...
  const scenarioResults = scenarios.map(scenario => {
    // Calculate effective salary after pension
    const effectiveSalary = scenario.salary * (1 - (scenario.pensionContribution || 0) / 100);
    const result = calculateTaxDetails(effectiveSalary, scenario.taxCode, true);
    ...
    return {
      ...
      pensionAmount: scenario.salary * ((scenario.pensionContribution || 0) / 100),
      taxCode: scenario.taxCode,
      incomeTax: result.annualSummary.totalIncomeTax,
      nationalInsurance: result.annualSummary.totalNI,
      totalDeductions: result.annualSummary.totalIncomeTax + result.annualSummary.totalNI,
      netPay: result.annualSummary.netAnnual,
      takeHomePercent: (result.annualSummary.netAnnual / scenario.salary) * 100
    };
  });
  ...
  const mostEfficientScenario = [...scenarioResults].sort((a, b) => b.takeHomePercent - a.takeHomePercent)[0];
          ...
          {activeTab === 'table' ? (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {scenarioResults.map((result) => (
                  <div key={result.id} className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 overflow-hidden">
                      ...
                      <dl className="space-y-2 mb-3">
                        ...
                        {result.pensionContribution > 0 && (
                          <div className="flex justify-between">
                            ...
                            <dd className="text-sm font-medium text-gray-900 dark:text-white">
                              {result.pensionContribution}% ({formatCurrency(result.pensionAmount)})
                            </dd>
                          </div>
                        )}
                      </dl>
                    ...
                    <div className="px-4 py-3 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex justify-between">
                      ...
                      <div className="space-x-3">
                        <button
                          onClick={() => handleDuplicateScenario(scenarios.find(s => s.id === result.id)!)}
                          className="text-sm text-govuk-blue hover:text-govuk-blue/80 dark:text-govuk-blue dark:hover:text-govuk-blue/80"
                        >
                          Duplicate
                        </button>
                        <button
                          onClick={() => handleEditScenario(scenarios.find(s => s.id === result.id)!)}
                          className="text-sm text-govuk-blue hover:text-govuk-blue/80 dark:text-govuk-blue dark:hover:text-govuk-blue/80"
                        >
                          {t('common.edit')}
                        </button>
              ...
              {comparisonScenarios.length > 0 && (
                <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead>
                        <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Net Pay Difference</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Income Tax Diff</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">NI Diff</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Monthly Gain/Loss</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {comparisonScenarios.map((scenario) => {
                          const netDiff = scenario.netPay - baseScenario.netPay;
                          const taxDiff = scenario.incomeTax - baseScenario.incomeTax;
                          const niDiff = scenario.nationalInsurance - baseScenario.nationalInsurance;
                          ...
                          return (
                            <tr key={scenario.id}>
                              ...
                              <td className={`px-6 py-4 whitespace-nowrap text-sm ${
                                ...
                                {niDiff > 0 ? '+' : ''}{formatCurrency(niDiff)}
                              </td>
                              <td className={`px-6 py-4 whitespace-nowrap text-sm font-medium ${
                                monthlyDiff > 0 
                                  ? 'text-green-600 dark:text-green-400' 
                  ...
                  <div className="mt-6 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                    <h4 className="text-sm font-medium text-blue-800 dark:text-blue-300 mb-2">Analysis & Insights</h4>
                    <div className="text-sm text-blue-700 dark:text-blue-200 space-y-2">
                      {mostEfficientScenario.id !== baseScenario.id && (
                        <p>
                          <strong>{mostEfficientScenario.name}</strong> is the most tax-efficient scenario, 
                          letting you keep {mostEfficientScenario.takeHomePercent.toFixed(1)}% of your gross income 
                          compared to {baseScenario.takeHomePercent.toFixed(1)}% in your base scenario.
                        </p>
                      )}
                      
                      {comparisonScenarios.some(s => s.pensionContribution > 0) && (
                        <p>
                          Pension contributions provide tax relief by reducing your taxable income, 
                          which can be especially beneficial for higher rate taxpayers.
                        </p>
                      )}
                      
                      {comparisonScenarios.some(s => s.taxCode !== baseScenario.taxCode) && (
                        <div>
                          <p>
                            Different tax codes can significantly impact your take-home pay. 
                            Ensure your tax code is correct to avoid underpaying or overpaying tax.
                          </p>
                          <p className="mt-2">
                            <button 
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="space-y-8">
                ...
                <div>
                  <h4 className="text-base font-medium text-gray-700 dark:text-gray-300 mb-3">Gross vs Net Income</h4>
                      ...
                      <div key={result.id} className="space-y-1">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-gray-700 dark:text-gray-300 font-medium">
                            {formatCurrency(result.salary)} → {formatCurrency(result.netPay)}
                          </span>
                        </div>
                        ...
                        <div className="relative pt-1">
                          <div className="flex h-6 overflow-hidden text-xs rounded-md bg-gray-200 dark:bg-gray-700">
                            ...
                            >
                              <span className="px-2 text-xs">{result.takeHomePercent.toFixed(1)}%</span>
                            </div>
                            <div
                              style={{ width: `${100 - result.takeHomePercent}%` }}
                            ...
                            >
                              <span className="px-2 text-xs">{(100 - result.takeHomePercent).toFixed(1)}%</span>
                            </div>
                          </div>
                          <div className="flex text-xs mt-1">
                            <span className="flex-1 text-left text-govuk-blue dark:text-govuk-blue">Take-home pay</span>
                            <span className="flex-1 text-right text-red-500 dark:text-red-400">Tax & NI</span>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div>
                  <h4 className="text-base font-medium text-gray-700 dark:text-gray-300 mb-3">Deductions Breakdown</h4>
                      ...
                      <div key={result.id} className="space-y-1">
                        ...
                        <div className="relative pt-1">
                          <div className="flex h-6 overflow-hidden text-xs rounded-md bg-gray-200 dark:bg-gray-700">
                            ...
                            <div
                              style={{ 
                                width: `${(result.incomeTax / result.totalDeductions) * 100}%` 
                              }}
                              className="flex flex-col justify-center text-center text-white bg-red-500 dark:bg-red-700 overflow-hidden whitespace-nowrap transition-all duration-500"
                            >
                            ...
                            <div
                              style={{ 
                                width: `${(result.nationalInsurance / result.totalDeductions) * 100}%` 
                              }}
                ...
                <div>
                  <h4 className="text-base font-medium text-gray-700 dark:text-gray-300 mb-3">Monthly Take-home Pay</h4>
                      ...
                      <div key={result.id} className="space-y-1">
                        ...
                        <div className="relative pt-1">
                          <div className="flex h-8 overflow-hidden text-xs rounded-md bg-gray-200 dark:bg-gray-700">
                            <div
                              style={{ 
                                width: `${(result.netPay / Math.max(...scenarioResults.map(s => s.netPay))) * 100}%` 
                              }}
                              className="flex flex-col justify-center text-center text-white bg-govuk-blue dark:bg-govuk-blue overflow-hidden whitespace-nowrap transition-all duration-500"
                            >
                              <span className="px-2 text-xs">{formatCurrency(result.netPay / 12)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
