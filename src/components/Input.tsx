...
export function Input({
  ...
  return (
    <div className={clsx("space-y-1", containerClassName)}>
        ...
        <input
          className={clsx(
            ...
            'dark:bg-gray-800 dark:text-white',
            {
              'pl-7': prefix,
              'pr-7': suffix,
              'border-red-300 dark:border-red-700 focus:ring-red-500 focus:border-red-500': error,
      ...
      {error && (
        <p className="mt-1 text-sm text-red-600 dark:text-red-400" id={`${props.id}-error`}>{error}</p>
      )}
    </div>
  );
}
